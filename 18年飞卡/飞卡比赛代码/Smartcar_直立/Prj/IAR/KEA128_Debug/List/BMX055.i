...
uint8 BMX055_init(void)
{
	...
	uint8 ErrCount = 0;
	  while(IIC_Read_Reg(0x18, 0x00) != 0xFA)   
	  {
	     ErrCount++;
	    
	    
	  }
	if (IIC_Write_Reg(0x18, 0x0F, 0x05) == 0)return 0;   
	soft_delay_ms(10);
	if (IIC_Write_Reg(0x18, 0x10, 0x0F) == 0)return 0;     
	soft_delay_ms(10);
	if (IIC_Write_Reg(0x18, 0x11, 0x00) == 0)return 0;   



	ErrCount = 0;
	while (IIC_Read_Reg(0x68, 0x00) != 0x0F)   
	{
		ErrCount++;
		if (ErrCount > 5)
			return 0;
	}
	if (IIC_Write_Reg(0x68, 0x0F, 0x01) == 0)return 0;   
	soft_delay_ms(10);
	if (IIC_Write_Reg(0x68, 0x10, 0x02) == 0)return 0;     
	soft_delay_ms(10);
	if (IIC_Write_Reg(0x68, 0x11, 0x00) == 0)return 0;   
	soft_delay_ms(10);
	if (IIC_Write_Reg(0x68, 0x13, 0x08) == 0)return 0;   
	soft_delay_ms(10);


	ErrCount = 0;
	IIC_Write_Reg(0x10, 0x4B, 0x81);
	soft_delay_ms(10);
	while (IIC_Read_Reg(0x10, 0x40) != 0x32)   
	{
		ErrCount++;
		if (ErrCount > 5)
			return 0;
	}
	if (IIC_Write_Reg(0x10, 0x4C, 0x38) == 0)return 0;   
	soft_delay_ms(10);
	if (IIC_Write_Reg(0x10, 0x4E, 0x00) == 0)return 0;      
	soft_delay_ms(10);
	return 1;
}
...
uint8 BMX055_DataRead(BMX055Datatypedef *Q, uint8 type)
{
	...
	if (IIC_Read_Buff(0x68, 0x02, datatemp, 6) == 0)return 0;
	...
	if (IIC_Read_Buff(0x18, 0x02, datatemp, 6) == 0)return 0;
	...
	if (type)
	{
		if (IIC_Read_Buff(0x10, 0x42, datatemp, 6) == 0)return 0;
