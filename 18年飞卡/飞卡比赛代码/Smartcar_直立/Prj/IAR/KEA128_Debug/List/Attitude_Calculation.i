...
void Quaternion_Normalize(QuaternionTypedef *Qu)       
{
	...
	if (__c99_generic(Normal,,, __iar_isnan64, __iar_isnan32, __iar_isnan64,,,)(Normal) || Normal <= 0)
	{
		Qu->W = 1;
		Qu->X = 0;
		Qu->Y = 0;
...
{
	if ((-Acc . Xdata) != 0 || (Acc . Zdata) != 0 || (-Acc . Ydata) != 0)
	{
		QuaternionFromAcc(&Quaternion, (-Acc . Xdata), (Acc . Zdata), (-Acc . Ydata), -1, 0, 0);
	}
	else
	{
		Quaternion.W = 1;
...
void Attitude_UpdateAcc(void)
{
	...
	QuaternionFromAcc(&MeaQuaternion, 0, (Acc . Zdata), (-Acc . Ydata), -1, 0, 0);
	...
	if (ComAxisangle.W > ((float)1 / 180 * 3.14159265358979f / (float)200))
	{
		ComAxisangle.W = ((float)1 / 180 * 3.14159265358979f / (float)200);
	}
...
void Attitude_UpdateGyro(void)  
{
	...
	float gx = (Gyro . Xdata) / 180 * 3.14159265358979f;
	float gy = (-Gyro . Zdata) / 180 * 3.14159265358979f;
	float gz = (Gyro . Ydata) / 180 * 3.14159265358979f;

	Quaternion_UpdateFromGyro(&Quaternion, gx, gy, gz, 0.005);
	Quaternion_ToEulerAngle(&Quaternion, &EulerAngle);
	Quaternion_ToAxisAngle(&Quaternion, &AxisAngle);
	...
	if (fabs(LastEulerAngle.Pitch - EulerAngle.Pitch) < 3.14159265358979f / 2)
	{
		EulerAngleRate.Pitch = EulerAngle.Pitch - LastEulerAngle.Pitch;
	}
	else if (EulerAngle.Pitch - LastEulerAngle.Pitch > 3.14159265358979f / 2)
	{
		EulerAngleRate.Pitch = -3.14159265358979f + (EulerAngle.Pitch - LastEulerAngle.Pitch);
	}
	else if (EulerAngle.Pitch - LastEulerAngle.Pitch < -3.14159265358979f / 2)
	{
		EulerAngleRate.Pitch = 3.14159265358979f + (EulerAngle.Pitch - LastEulerAngle.Pitch);
