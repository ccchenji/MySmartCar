...
void OLED_HEXACSII(uint16 hex,uint8* Print,uint8 TEMP);
void OLED_Print_Num(uint8 x, uint8 y, uint16 num ,uint8 bit);
void OLED_Print_Num1(uint8 x, uint8 y, int16 num ,uint8 bit);
void dis_bmp(uint16 high, uint16 width, uint8 *p,uint8 value);
void pixelC_HW_OLED_FillScreen(uint8 bmp[128][8]);
...
void OLED_WrDat(uint8 data)
{
	...
	while(i--)
	{
	  	if(data&0x80){gpio_set (PTG0 , 1);}
	  	else{gpio_set (PTG0 , 0);}
	  	gpio_set (PTE1 , 1);
