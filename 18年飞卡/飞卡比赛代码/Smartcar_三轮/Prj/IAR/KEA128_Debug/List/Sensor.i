...
uint16 sensor_value[6] =        {0,   0,   0,   0,   0,   0};  
uint16 normalization_threshold[6] = {0,0,0,0,0,0};              
extern uint8 loss_line;                 
extern uint8 loss_line_lock;            
...
float cal_deviation(uint16 * sensor_value)
{
	...
	for(uint8 i = 0 ; i<6 ; i++)
	{
		sensor_value_copy[i] = (float)(sensor_value[i]);		
	}
	 
	sensor_value_copy[0] += 2.5f;		
	...
	pwr_total = pwr[0] +pwr[1] + pwr[2];		

	kp[0] =((float)(pwr[0])) / ((float)(pwr_total));			
	kp[1] =((float)(pwr[1])) / ((float)(pwr_total));			
	kp[2] =((float)(pwr[2])) / ((float)(pwr_total));			
...
void loss_line_deal(uint16 * sensor_value , float position ,uint16 left_lose_threshold ,uint16 right_lose_threshold)
{
        ...
        for(uint8 i = 2 ; i>0 ; i--)
        {
            error_add[i] = error_add[i-1];              
        }
        error_add[0] = (int)(position * 100);           
        error_total = error_add[0]  + error_add[1] + error_add[2];      
        sensor_value_total = sensor_value[0] + sensor_value[2] + sensor_value[3] + sensor_value[5];                        
        
        if((sensor_value_total < left_lose_threshold) || (sensor_value_total < right_lose_threshold))            
        {
...
uint8 out_line_deal(uint16 * sensor_value , float position)
{
    ...
    if((int)(position) > 0)
    {
        ...
        {
            if((((sensor_value_minus) > 0) ? (sensor_value_minus) : (-(sensor_value_minus))) < 2)
            {
                return 1;                       
            }
        }
...
float position_filter(float position)
{
        ...
        for( i = 9 ; i>0 ; i--)
        {
            position_add[i] = position_add[i-1];              
        }
        position_add[0] = position;                           
        ...
        if(position_average > 200)                              
        {
          position_average = 200;
        }
        if(position_average < (-(200)))
        {
          position_average = (-(200));
...
float position_record(float position)
{
    ...
    for( i = 14 ; i>0 ; i--)
    {
        position_accumulative[i] = position_accumulative[i-1];              
    }
