...
}unsigned char Key_Scan(void)
{
	...
	if (key_get(KEY_A) == KEY_DOWN) { num = 0x60; }
	if (key_get(KEY_B) == KEY_DOWN) { num = 0x40; }
	if (key_get(KEY_D) == KEY_DOWN) { num = 0x20; }
	if (key_get(KEY_U) == KEY_DOWN) { num = 0x50; }
	if (key_get(KEY_L) == KEY_DOWN) { num = 0x30; }
	if (key_get(KEY_R) == KEY_DOWN) { num = 0x10; }
...
void Speed_calulate(void)
{
	motorEncorderL = ftm_pulse_get(((FTM_Type *)(0x40039000u)));
	ftm_clean(((FTM_Type *)(0x40039000u)));
	if (gpio_get(PTD2) == 0)
		motorEncorderL = - motorEncorderL;
        
	motorEncorderR = ftm_pulse_get(((FTM_Type *)(0x40038000u)));
...
void Encoder_init(void)
{
    ...
    ftm_pulse_init(((FTM_Type *)(0x40038000u)),FTM_PS_4,PTE7);
    ftm_pulse_init(((FTM_Type *)(0x40039000u)),FTM_PS_4,PTE0);
}
...
void Motor_control(void)
{
	...
	if (dutyL < 0)
	{
		ftm_pwm_duty(((FTM_Type *)(0x4003A000u)), FTM_CH0, 250);
	...
	else if (dutyL >= 0)
	{
		...
		ftm_pwm_duty(((FTM_Type *)(0x4003A000u)), FTM_CH1, dutyL);
	}

	if (dutyR < 0)
	{
		ftm_pwm_duty(((FTM_Type *)(0x4003A000u)), FTM_CH2, 0);
		ftm_pwm_duty(((FTM_Type *)(0x4003A000u)), FTM_CH3, 250);
	}
	else if (dutyR >= 0)
	{
		ftm_pwm_duty(((FTM_Type *)(0x4003A000u)), FTM_CH2, dutyR);
...
void Dir_PdControl(void)
{
	dir.err = adc_value[0]-adc_value[3];
...
int ADC_deal(int * adcValue)
{
	...
	adc_cal[0]=ValueCopy[0]-ValueCopy[1];
	adc_cal[1]=ValueCopy[1]-ValueCopy[2];
	adc_cal[2]=ValueCopy[2]-ValueCopy[3];
	adc_cal[3]=ValueCopy[0]-ValueCopy[3];
...
void lose_line_deal(int *adc_Value, int position, int leftRange, int rightRange)
{
	...
	error_add[0] = position;								  
	error_tatal = error_add[0] + error_add[1] + error_add[2]; 
	sensor_value_all = adc_value[0] + adc_value[1] + adc_value[2] + adc_value[3];
	if ((g_leftThrottle > sensor_value_all) || (g_rightThrottle < fabs(sensor_value_all)) && (adc_value[0]<10||adc_value[3]<10))
	{
...
int get_sensor_threshold_normalization(void)
{
	...
	for (int i = 0; i < 4; i++)
	{
		if (sensor_value_max[i] < ( ((sensor_value_now[i]) > (sensor_value_last[i])) ? (sensor_value_now[i]) : (sensor_value_last[i]) ))
		{
			sensor_value_max[i] = ( ((sensor_value_now[i]) > (sensor_value_last[i])) ? (sensor_value_now[i]) : (sensor_value_last[i]) );
		}
	}
	for (int i = 0; i < 4; i++)
	{
...
void sensorValue_get(int * sensor_value)
{
	sensor_value[0] = adc_once(ADC0_SE12, ADC_10bit);
	sensor_value[1] = adc_once(ADC0_SE13, ADC_10bit);
	sensor_value[2] = adc_once(ADC0_SE14, ADC_10bit);
	sensor_value[3] = adc_once(ADC0_SE15, ADC_10bit);
}
