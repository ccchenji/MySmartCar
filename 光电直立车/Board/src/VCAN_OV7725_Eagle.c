...
void ov7725_eagle_port_init()
{
	...
	DMA_DIS(CAMERA_DMA_CH);					//禁止MDA硬件请求
	disable_irq(PORTA_IRQn);                //禁止场中断
	DMA_IRQ_CLEAN(CAMERA_DMA_CH);           //清除通道传输中断标志位
	DMA_IRQ_EN(CAMERA_DMA_CH);				//允许DMA通道传输完成中断(常开不关)

	port_init(PTA27, ALT1 | DMA_FALLING | PULLDOWN);         //PCLK MDA硬件请求
...
{
	//场中断需要判断是场结束还是场开始                  
	if (ov7725_eagle_img_flag == IMG_FINISH)                  //需要开始采集图像
	{
	...
	else if (ov7725_eagle_img_flag == IMG_START)              //需要开始采集图像
	{
		ov7725_eagle_img_flag = IMG_GATHER;                  //标记图像采集中
		DMA_EN(CAMERA_DMA_CH);//使能通道CHn 硬件请求
		DMA_DADDR(CAMERA_DMA_CH) = (uint32)ov7725_eagle_img_buff;    //恢复地址
...
/*!
*  @brief      鹰眼ov7725采集图像（采集到的数据存储在初始化时配置的地址上）
*  @since      v5.0
*/
void ov7725_eagle_get_img()
{
	...
	ov7725_eagle_img_flag = IMG_START;                   //开始下次采集图像
}
